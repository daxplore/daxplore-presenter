(function (namespace) {
  ...
  // Takes a canvas, standardizes margins, populates & adds the watermark text and returns a new canvas.
  const imageScaling = 2 // TODO externalize
  common.composeImageFromCanvas =
  function (canvas, watermarkText) {
    // Define margins
    const margin = {
  ...
  // Takes a dataurl generated by domtoimage, watermark text ID and filename text ID
  // Normalizes margins and adds the watermark then generates a "save image" dialog for the end user
  common.composeAndSaveImage =
  function (dataUrl, watermarkID, filenameID) {
    const img = new Image()
    img.onload = function () {
      ...
      // Define watermark
      const watermarkText = dax.text(watermarkID)
      const compositeCanvas = common.composeImageFromCanvas(initialCanvas, watermarkText)

      compositeCanvas.toBlob(function (blob) {
        saveAs(blob, dax.text(filenameID) + '.png')
      })
