(function (namespace) {
  ...
  exports.populateChart = function (stat, selectedPerspectiveOptionsInput) {
    ...
    rows.select('.legend__row-text')
      .text(function (option) {
        ...
        if (option.nodata) {
          text += ' ' + daxplore.text('meanbars_legend_missingData')
        }
        return text
      })
    ...
    legendReferenceLine
      .text(daxplore.text('meanbars_legend_referenceValue', daxplore.common.integerFormat(questionReferenceValue)))
    ...
    referenceTooltipDiv.append('span')
      ...
      .text(daxplore.text('meanbars_tooltip_referenceValue'))
    referenceTooltipDiv.append('span')
  ...
  function optionMouseOver (hoveredOption, showTooltip) {
    ...
    if (data.nodata) {
      tooltipText += daxplore.text('meanbars_tooltip_fewRespondents', 10) + '<br>' // TODO hardcoded 10, should be read from settings generated by Daxplore Producer
      tooltipText += '<b>' + daxplore.text('meanbars_tooltip_missingData') + '</b>'
    } else {
      tooltipText += daxplore.text('meanbars_tooltip_mean', daxplore.common.integerFormat(data.mean)) + '<br>' +
      daxplore.text('meanbars_tooltip_respondents', data.count)
    }
