(function (namespace) {
  ...
  exports.populateChart = function (questionID, perspectiveID, selectedPerspectiveOptions) {
    ...
    legendReferenceLine
      .text(dax.text('meanbars_legend_referenceValue', dax.common.integerFormat(questionReferenceValue)))
  ...
  function optionMouseOver (hoveredOption, showTooltip) {
    ...
    if (data.nodata) {
      ...
      tooltipText += dax.text('meanbars_tooltip_mean', dax.common.integerFormat(data.mean)) + '<br>' +
      dax.text('meanbars_tooltip_respondents', data.count)
    }
