 ...
 * www.source-code.biz/base64coder/java</a><br> Author: Christian d'Heureuse,
 * Inventec Informatik AG, Zurich, Switzerland<br> Multi-licensed: EPL / LGPL /
 * GPL / AL / BSD / MIT.
 */
public class Base64 {
	...
	/**
	 * Encode a long as a Base64 String.
	 * 
	 * <p>Added for the Daxplore project.</p>
	 * 
	 * @param in
	 *            A long to be encoded.
	 * @return A String containing the Base64 encoded data.
	 */
	...
	/**
	 * Decode a Base64 String as a long.
	 * 
	 * <p>Added for the Daxplore project.</p>
	 * 
	 * @param in
	 *            A Base64 String to be decoded.
	 * @return A long containing the decoded Base64 data.
	 */
	public static long decodeLong(String in) {
		byte[] decoded = decode(in);
		if (decoded.length > 8) {
			throw new IllegalArgumentException(
					"Length of Base64 encoded input string contains more bits than what fits in a long.");
		}
		long result = 0;
		for (int i = 0; i < decoded.length; i++) {
			result <<= 8;
			result |= (decoded[i] & 0xFF);
		}
		return result;
	}
